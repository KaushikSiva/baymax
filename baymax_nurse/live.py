...
class _StaticHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format: str, *args: object) -> None:
        logger.debug("Baymax HTTP: " + format, *args)


class HospitalLiveServer:
    def __init__(
        self,
        *,
        host: str = "127.0.0.1",
    ...
    def public_url(self) -> str:
        return f"{self.url}/?wsPort={self.websocket_port}"

    def start(self) -> None:
        handler = type("BaymaxStaticHandler", (_StaticHandler,), {"web_root": self.web_root})
        self._http = ThreadingHTTPServer((self.host, self.http_port), handler)
        self._http_thread = threading.Thread(
            target=self._http.serve_forever, daemon=True, name="baymax-http"
        )
        self._http_thread.start()
        self._ws_thread = threading.Thread(
            target=self._run_websocket, daemon=True, name="baymax-ws"
        )
        self._ws_thread.start()
        if not self._started.wait(timeout=5):
            raise RuntimeError("Baymax WebSocket server did not start")
    ...
    def publish(self, state: dict[str, Any], frames: dict[str, bytes] | None = None) -> None:
        ...
        if self._ws_loop is not None and self._ws_loop.is_running():
            asyncio.run_coroutine_threadsafe(
                self._broadcast(self._latest_message), self._ws_loop
            )
